...
// Singleton (one-per-case) columns on the `cases` table. Excludes id,
// case_number, created_at and updated_at which are managed internally.
const CASE_COLUMNS = [
  // General
...
// Child tables: { jsonKey, table, columns, booleans }
const CHILD_TABLES = [
  {
    key: 'reporters',
    table: 'reporters',
...
// Products are special: they have nested dosages.
...
function buildInsert(table, columns) {
  const cols = ['case_id', 'sort_order', ...columns];
...
for (const child of CHILD_TABLES) {
  childInserts[child.table] = buildInsert(child.table, child.columns);
}
const productInsert = buildInsert('products', PRODUCT_COLUMNS);
const dosageInsert = (() => {
  const cols = ['product_id', 'sort_order', ...DOSAGE_COLUMNS];
  const placeholders = cols.map((c) => `@${c}`).join(', ');
  return db.prepare(
    `INSERT INTO dosages (${cols.join(', ')}) VALUES (${placeholders})`
  );
})();
...
function insertChildren(caseId, data) {
  for (const child of CHILD_TABLES) {
    ...
    rows.forEach((item, index) => {
      ...
      childInserts[child.table].run(row);
    });
  }

  // Products + their nested dosages
  const products = Array.isArray(data.products) ? data.products : [];
  products.forEach((product, pIndex) => {
    const row = rowFromObject(product, PRODUCT_COLUMNS, new Set());
    row.case_id = caseId;
    row.sort_order = pIndex;
    const info = productInsert.run(row);
...
function deleteChildren(caseId) {
  ...
  // Deleting products cascades to dosages via the foreign key.
...
function listCases() {
  return db
    ...
    .map((c) => {
      const events = db
        .prepare('SELECT pt_term, description_reported, is_serious FROM events WHERE case_id = ? ORDER BY sort_order')
        .all(c.id);
      const products = db
        .prepare('SELECT product_name FROM products WHERE case_id = ? AND drug_type = ? ORDER BY sort_order')
        .all(c.id, 'Suspect');
      return {
        ...c,
        primary_event:
...
function getCase(id) {
  ...
  for (const child of CHILD_TABLES) {
    caseRow[child.key] = db
      .prepare(`SELECT * FROM ${child.table} WHERE case_id = ? ORDER BY sort_order`)
      .all(id);
  }

  const products = db
    .prepare('SELECT * FROM products WHERE case_id = ? ORDER BY sort_order')
    .all(id);
  for (const product of products) {
    product.dosages = db
      .prepare('SELECT * FROM dosages WHERE product_id = ? ORDER BY sort_order')
      .all(product.id);
  }
...
const createCase = db.transaction((data) => {
  ...
  for (const col of CASE_COLUMNS) {
    row[col] = coerce(data[col], CASE_BOOLEANS.has(col));
  }
  row.case_number = caseNumber;
  row.created_at = now;
  row.updated_at = now;

  const cols = ['case_number', ...CASE_COLUMNS, 'created_at', 'updated_at'];
  const placeholders = cols.map((c) => `@${c}`).join(', ');
  const info = db
    .prepare(`INSERT INTO cases (${cols.join(', ')}) VALUES (${placeholders})`)
    .run(row);

  const caseId = info.lastInsertRowid;
  insertChildren(caseId, data);
  return getCase(caseId);
});

const updateCase = db.transaction((id, data) => {
